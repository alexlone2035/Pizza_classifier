...
Проект покрыт интеграционными тестами с использованием фреймворка `pytest`. 
Тесты прогоняют полный пайплайн на реальных весах моделей, проверяя работоспособность детектора, классификатора и главного оркестратора.

Для быстрого запуска тестов (веса загружаются в память только один раз благодаря фикстурам):
```bash
pytest test_pizza_pipeline.py -v
